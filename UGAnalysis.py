...
if set_self:
    ...
    explore = 0.01       # with prob [explore], agents adapt strategy randomly. prob [1 - explore] = unconditional/proportional imit
...
#for g in graphType:
for selectionStyle in ['unconditional']:
    g = 'Barabasi-Albert'
    ...
    if testCase:
        gameAna = pd.read_csv("Data/gameTest_n{0}_sim{1}_round{2}_exp={3:.2f}_random={4}_select={5}_beta={6}.csv".format(agentCount, simulations, rounds, explore, str(randomRoles), selectionStyle, selectionIntensity), encoding='utf-8', header = [0,1,2])
        edgeAna = pd.read_csv("Data/edgeTest_n{0}_sim{1}_round{2}_exp={3:.2f}_random={4}_select={5}_beta={6}.csv".format(agentCount, simulations, rounds, explore, str(randomRoles), selectionStyle, selectionIntensity), encoding='utf-8', header = [0,1,2])
    else:
        gameAna = pd.read_csv("Data/gameData_n{0}_sim{1}_round{2}_exp={3:.2f}_random={4}_select={5}_beta={6}.csv".format(agentCount, simulations, rounds, explore, str(randomRoles), selectionStyle, selectionIntensity), encoding="utf-8", header = [0,1,2])
        edgeAna = pd.read_csv("Data/edgeData_n{0}_sim{1}_round{2}_exp={3:.2f}_random={4}_select={5}_beta={6}.csv".format(agentCount, simulations, rounds, explore, str(randomRoles), selectionStyle, selectionIntensity), encoding='utf-8', header = [0,1,2])
    ...
    dat, p, q = np.histogram2d(initq, initp, bins=nbins, density=False)#, range=[[0, 1], [0, 1]])
    ext = [q[0], q[-1], p[0], p[-1]]
    ...
    #p_kde = sns.kdeplot(initp, ax=ax5, shade=True, color='r', bw=0.1, clip=(0, 1), label="p")
    sns.kdeplot(initp, ax=ax5, color='r', bw=0.1, clip=(0, 1), label="p")
    #q_kde = sns.kdeplot(initq, ax=ax5, shade=True, color='b', bw=0.1, clip=(0, 1), label="q")
    sns.kdeplot(initq, ax=ax5, color='b', bw=0.1, clip=(0, 1), label="q")
    ax5.set_xlim(left = 0, right = 1) 
    
    #writer = ani.FFMpegWriter(fps=11, metadata=dict(artist='Me'), bitrate=1800)
    ...
    def run_animation():
        ...
        def init():
            ...
            return lines,#lines[0] # lines[1], lines[2], #[0], lines[1], lines[2])# im
        ...
        def animate(currentRound):#, lines):#offerlist, acceptlist, successlist, lines):
            ...
            #nx.draw_networkx_nodes(graph, pos = positions, ax=axgraph, node_color=nodecol, alpha = 0.53, node_size = 1200, with_labels=True, font_size = 30)
            #nx.draw_networkx_edges(graph, pos = positions, ax=axgraph, edge_color = edgecol, edge_cmap = plt.cm.coolwarm, edge_vmin=0, edge_vmax=(2*rounds), width = edgesize)
    ...
    def save_animation():
        ...
        outf = "Videos/test_{0}V{1}_n{2}_sim{3}_round{4}_exp={5:.2f}_random={6}_select={7}_beta={8}.mp4".format(g, sim, agentCount, simulations, rounds, explore, str(randomRoles), selectionStyle, selectionIntensity)
        cmdstring = ('ffmpeg', 
                 '-y', '-r', '11', # overwrite, 11fps
                 '-s', '%dx%d' % (canvas_width, canvas_height), # size of image string
                 '-pix_fmt', 'argb', # format
                 '-f', 'rawvideo',  '-i', '-', # tell ffmpeg to expect raw video from the pipe
                 '-vcodec', 'mpeg4', '-b:v', '7000k', outf) # output encoding
        pipp = subprocess.Popen(cmdstring, stdin=subprocess.PIPE)
    
        def aniupdate(currentRound):#, lines):#offerlist, acceptlist, successlist, lines):
            #since edge interaction 2x per round, animate edge round 2x currentRound? (set frames = rounds times two, maintain currentRound by (frames/2))
